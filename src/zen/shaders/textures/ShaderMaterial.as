        ...
        private function uploadPrograms(programs:Vector.<ShaderProgram>):void
        {
            ...
            while (p < programs.length) {
                ...
                for each (sampler in prg.samplers) {
                    ...
                    if (!(sampler.value)){
                        if (sampler.request){
                            libTexture = (scene.library.getItem(sampler.request) as Texture3D);
                            if (((libTexture) && ((libTexture.typeMode == sampler.type)))){
                                sampler.value = libTexture;
                            } else {
                                sampler.value = new Texture3D((currentTextuesPath + sampler.request), sampler.optimizeForRenderToTexture, sampler.format, sampler.type);
                            }
                            scene.library.addItem(sampler.request, sampler.value);
                        } else {
                            if ((((sampler.width > 0)) && ((sampler.height > 0)))){
                                sampler.value = new Texture3D(new Point(sampler.width, sampler.height), sampler.optimizeForRenderToTexture, sampler.format, sampler.type);
                            } else {
                                sampler.value = new Texture3D(ZenUtils.nullBitmapData, sampler.optimizeForRenderToTexture, sampler.format, sampler.type);
                            }
                        }
                    }
