package zen.geom
{
	 ...
	 * Modifying these values have no effect on the objects.
	 * The values returned are represented in the local space of each object.
	 * To transform them to a global space use localToGlobal or localToGlobalVector of the ZenObject objects.
	 * 
	 * Although values can be modified, this is not advisable as these properties are often used by the visibility 
	 * and collision calculation system, for example. Preferably, create a clone() and modify that copy.
	 */
	public class Cube3D 
    {
       ...
       /// Center of the object. The center does not correspond to (0,0,0) of the object, but it is the calculated center of all the vertices.
		...
		public function cornerPoints():Array {
			return [
				new Vector3D(min.x, min.y, min.z), // Top Front Left
