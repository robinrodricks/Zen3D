package zen.display
{
    ...
    public class ZenSkyBox extends ZenMesh 
    {
        ...
        public function ZenSkyBox(request, format:String="horizontalCross", sceneContext:Zen3D=null, scaleRatio:Number=0.5)
        {
            ...
            var ext:String;
            var images:Array;
            this._textures = new Vector.<Texture3D>();
            super("skybox");
            ...
            if ((((format == HORIZONTAL_CROSS)) || ((format == VERTICAL_CROSS)))){
                i = 0;
                while (i < 6) {
                    this._textures[i] = new Texture3D(ZenUtils.nullBitmapData);
                    i++;
                }
                this._texture = new Texture3D(request);
                ...
                if ((((format == FOLDER_JPG)) || ((format == FOLDER_PNG)))){
                    ...
                    i = 0;
                    while (i < 6) {
                        this._textures[i] = (this._library.push(new Texture3D(((folder + images[i]) + ext))) as Texture3D);
                        i++;
                    }
                } else {
                    if (format == BITMAP_DATA_ARRAY){
                        this._textures[0] = new Texture3D(request[0]);
                        this._textures[1] = new Texture3D(request[2]);
                        this._textures[2] = new Texture3D(request[1]);
                        this._textures[3] = new Texture3D(request[3]);
                        this._textures[4] = new Texture3D(request[4]);
                        this._textures[5] = new Texture3D(request[5]);
                    } else {
        ...
        private function completeTextureEvent(e:Event=null):void
        {
            ...
            switch (this._format){
                ...
                case VERTICAL_CROSS:
                    ...
                    i = 0;
                    while (i < 6) {
                        this._textures[i] = new Texture3D(new BitmapData(w, h, false));
                        i++;
                    }
