package zen.display
{
	...
	/**
	 * The Zen3D class is the main element, container and manager of all the 3d resources and objects.
	 * 
	 *   <p>The Zen3D class also has a Clock3D instance that manages update and render cycles. The update event indicates the need to update scene elements and the render and
	 * postRender events allow full control before and after the render process.</p><p>Once the scene has been created, use addChild or addChildFromFile to include 3d objects. This will dispatch the progress 
	 * and complete events to control the loading process for external files.</p><p>The scene also creates a ZenCamera object to view the scene.
	 ...
	 * 
	 *   // creates the scene.
	 * var scene:Zen3D = new Zen3D( this )
	 * 
	 *   // adds the events to manage the object loading process.  
	 * scene.addEventListener( Zen3D.PROGRESS_EVENT, progressEvent )
	 * scene.addEventListener( Zen3D.COMPLETE_EVENT, completeEvent )
	 * 
	 ...
	 * scene.addEventListener( Zen3D.UPDATE_EVENT, updateEvent )
	 * }
	 * 
    ...
    public class Zen3D extends ZenObject 
    {
		...
		/**
		 * Allows to import scene setting saved in the IDE. Once the settings are imported for the first time, this property is set to false automatically.
		 * This includes properties such as, lights technique, ambient color, clear color, number of lights, default light, gamma, fog, global shadows filtering.
		 */
        public var allowImportSettings:Boolean = true;
		...
		private var renderTex:Texture3D;
        ...
        public function drawQuadTexture(texture:Texture3D, x:Number, y:Number, width:Number, height:Number, material:ShaderMaterialBase=null, sourceFactor:String="sourceAlpha", destFactor:String="oneMinusSourceAlpha"):void
        {
            if (!(material)){
                ...
                if (!(this._paused)){
                    ...
                    if (this.targetTexture){
                        ...
                        if ((((length > 1)) && (!(this._secondTargetTexture)))){
                            this._secondTargetTexture = new Texture3D((this._targetTexture.request as Point), true);
		 ...
		 */
		public function render(camera:ZenCamera=null, clearDepth:Boolean=false, target:Texture3D=null):void
        {
        ...
        {
            var texture:Texture3D = new Texture3D(request, optimizeForRenderToTexture, format, type);
       ...
       /**
		...
		public function set renderToQuad(value:Boolean):void {
			if (_renderToQuad != value){
				...
				if (_renderToQuad) {
					//hide();
					renderTex = new Texture3D(new Rectangle(0, 0, 1024, 1024), true);
