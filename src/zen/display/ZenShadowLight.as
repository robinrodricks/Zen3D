package zen.display {
	...
	/** A 3D shadow projector that renders shadows behind objects it is pointed towards.
	 * Must be added to a Zen3D instance to take effect. */
	public class ZenShadowLight extends ZenLight {
		...
		private var _depthTexture:ZenTexture;
		...
		public function get depthTexture():ZenTexture {
		...
		private function build():void {
			...
			if (this.cascades == 1) {
				this._depthTexture = new ZenTexture(new Rectangle(0, 0, this.quality, this.quality), true);
			} else {
				if (this.cascades <= 2) {
					this._depthTexture = new ZenTexture(new Rectangle(0, 0, this.quality, (this.quality * 0.5)), true);
				} else {
					this._depthTexture = new ZenTexture(new Rectangle(0, 0, this.quality, this.quality), true);
				}
			}
		...
		public function renderShadowMap(srcFactor:String = "one", destFactor:String = "zero", shadowTexture:ZenTexture = null):void {
			var m:ZenMesh;
			var surfaces:int;
			var s:int;
			var surf:ZenFace;
		...
		public function renderDepthMap(depthTexture:ZenTexture = null):void {
