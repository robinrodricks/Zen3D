package zen.loaders {
	...
	public class AssetLoader extends EventDispatcher implements IAsset {
		...
		private var _loadInQueued:Vector.<IAsset>;
		...
		private var _items:Vector.<IAsset>;
		private var _path:Dictionary;
		
		public function AssetLoader(connections:int = 10, autoStart:Boolean = true, allowEventsAfterComplete:Boolean = true) {
			this._loadInQueued = new Vector.<IAsset>();
			this._loading = new Vector.<IAsset>();
			this._items = new Vector.<IAsset>();
		...
		public function addItem(key, item:IAsset):IAsset {
			this._path[key] = item;
		...
		public function removeItem(key):IAsset {
			var index:int;
			var item:IAsset = this._path[key];
			if (item) {
		...
		public function getItem(key):IAsset {
			return (this._path[key]);
		}
		
		public function push(item:IAsset):IAsset {
			if (item.loaded) {
				return (null);
			}
		...
		public function load():void {
			...
			var item:IAsset = this._loadInQueued.shift();
			item.load();
		...
		private function progressQueuedEvent(e:ProgressEvent):void {
			...
			for each (item in this._loading) {
				...
				if ((item is AssetLoader)) {
					this._progress = (this._progress + ((AssetLoader(item).progress / 100) / this._loading.length));
				} else {
		...
		public function get items():Vector.<IAsset> {
			return (this._items.concat());
		}
		
		public function get itemsToLoad():Vector.<IAsset> {
